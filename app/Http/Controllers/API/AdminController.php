            ...
            if (!empty($request->id)) {
                ...
                if ($check_user->count() > 0) {
                    ...
                    $users_info = [
                        ...
                        'today_sales' => $today_sales = DB::table('data')->where(['plan_status' => 1])->whereDate('plan_date', Carbon::today())->sum('amount') + DB::table('airtime')->where(['plan_status' => 1])->whereDate('plan_date', Carbon::today())->sum('discount'),

                        'yesterday_sales' => $yesterday_sales = DB::table('data')->where(['plan_status' => 1])->whereDate('plan_date', Carbon::yesterday())->sum('amount') + DB::table('airtime')->where(['plan_status' => 1])->whereDate('plan_date', Carbon::yesterday())->sum('discount'),
                        'yesterday_trans' => $yesterday_trans = DB::table('data')->where(['plan_status' => 1])->whereDate('plan_date', Carbon::yesterday())->count() + DB::table('airtime')->where(['plan_status' => 1])->whereDate('plan_date', Carbon::yesterday())->count(),

                        'sales_percent' => $yesterday_sales > 0 ? round((($today_sales - $yesterday_sales) / $yesterday_sales) * 100, 1) : ($today_sales > 0 ? 100 : 0),
                        'trans_percent' => $yesterday_trans > 0 ? round(((($today_data + $today_airtime) - $yesterday_trans) / $yesterday_trans) * 100, 1) : (($today_data + $today_airtime) > 0 ? 100 : 0),
