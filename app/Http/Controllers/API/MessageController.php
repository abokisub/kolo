            ...
            if (!empty($request->id)) {
                ...
                if ($check_user->count() > 0) {
                    ...
                    foreach ($all_user as $user) {
                        $change_habukhan = [$user->username, $user->email, $user->name, $user->phone, $user->webhook, $user->apikey, $user->address, $user->ref, $user->type, $user->wema, $user->rolex, $user->sterlen, $user->fed, $user->otp, $user->user_limit, '₦' . number_format($user->bal, 2), '₦' . number_format($user->refbal, 2)];
            ...
            if (!empty($request->id)) {
                ...
                if ($check_user->count() > 0) {
                    ...
                    foreach ($all_user as $user) {
                        ...
                        DB::table('notif')->insert(['username' => $user->username, 'message' => $real_message, 'date' => $this->system_date(), 'habukhan' => 0]);
                    }
                } else {
                    return response()->json([
                        'status' => 403,
